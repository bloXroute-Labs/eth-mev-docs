...
- `fee_payer_address` is an account which holds the ETH used by the relay to pay the fee recipient. Builders fund this account to use the feature. All adjusted bids are paid from this address. Contact the relay to set up an initial fee payer account.
...
See example of ultrasound's testnet builder with adjustments: https://github.com/blombern/rbuilder/tree/adjustments


### SSZ encoding

We strongly suggest SSZ (+ gzip) encoding all submissions for the best performance. The `AdjustmentData` is expected as the last field in `SubmitBlockRequest` http or grpc request (see JSON example above). For grpc encoding see for grpc encoding see https://github.com/bloXroute-Labs/relay-grpc.  Here's how to encode `AdjustmentData` as SSZ:
...
Ex:
https://bloxroute.max-profit.blxrbdn.com/blxr/v1/data/adjustments?slot=11155869
https://bloxroute.regulated.blxrbdn.com/blxr/v1/data/adjustments?slot=11155869

The API expects a single query parameter, slot, indicating the slot you'd like to see the adjustments for.
...
This documentation is based on the original bidadjustment documentation by ultrasound: https://github.com/ultrasoundmoney/docs/blob/main/bid-adjustment.md"
